...
export default class EventCalendar extends Component {
  ...
  componentDidMount() {
    if (new Authentication().isUserLoggedIn()) this.getAllEvents();
  }
  ...
  getAllEvents = () => {
    REQUEST_EVENT_GET({}, (err, res) => {
      if (err) {
        alert(
          "Could not load events due to authentication problem. Retrying..."
        );
      ...
      >
        ...
        <Modal.Body>
          ...
          <div>
            ...
            <span className="badge">{this.reformatDate(event.date.start)}</span>
          </div>
          <div>
            <strong>End Date: </strong>{" "}
            <span className="badge">{this.reformatDate(event.date.end)}</span>
          </div>
          <div>
            <strong>Location: </strong>{" "}
            <span className="badge">{event.location.name}</span>
          </div>
          <div style={{ height: "100vh", width: "100%" }}>
            <GoogleMapReact
              bootstrapURLKeys={{
                key: "AIzaSyDdGktaDO-ZTNobA1T512xE11KTvkCOc-o",
              }}
              defaultCenter={{
            ...
            >
              ...
              >
                <img
                  src="/images/icons8-marker-96.png"
